Shader "Unlit/LatLong"
{
	Properties
	{
		...
		_PanoramaMap("PanoramaMap",2D)="white"{}  //全景图
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				//采样环境贴图 
				float4 color_cubemap = tex2D(_PanoramaMap,uv_panorama);

				//确保在移动端显示HDR成功 需要进行解码
				float3 decode_cubemap = DecodeHDR(color_cubemap,_PanoramaMap_HDR);
